...
fn test_alloc_large_block() {
    ...
    match MemoryManager::alloc_large_block(size) {
        Ok(ptr) => {
            ...
            let result = MemoryManager::free_large_block(ptr, size);
...
fn test_free_invalid_memory() {
    ...
    let result = MemoryManager::free_large_block(invalid_ptr, size);
